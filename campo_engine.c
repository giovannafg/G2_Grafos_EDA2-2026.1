...
int main() {
    ...
    while (fgets(cmd, sizeof(cmd), stdin)) {
        if (strncmp(cmd, "QUIT", 4) == 0) {
            ...
            if (level == 1) { max = 10; num_bombas = 10; }
            else if (level == 2) { max = 15; num_bombas = 35; }
            else { max = 20; num_bombas = 90; }

            if (matriz_ptr) free(matriz_ptr);
            ...
            if (status == 0 && r >= 0 && r < max && c >= 0 && c < max) {
                if (matriz[r][c].flag == 0) {
                    if (matriz[r][c].boolean == 1) {
                        ...
                        if (vitoria(max, matriz, num_bombas)) { // Usa sua função de vitória
                            status = 1; 
                        }
                    }
                }
